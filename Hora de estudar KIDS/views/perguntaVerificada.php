...
<body>
  ...
  <div class="row">
    <?php
    if (isset($_SESSION['erros_CQ'])) {
      echo "<div class='alert alert-danger' style='height: 40px, width:70%;'>
        $_SESSION[erros_CQ]
      </div> ";
      unset($_SESSION['erros_CQ']);
    }
    if (isset($_SESSION['sucessos_CQ'])) {
      echo "<div class='alert alert-success' style='height: 50px;'>
          $_SESSION[sucessos_CQ]
      </div> ";
      unset($_SESSION['sucessos_CQ']);
    }
    ?>
  </div>
  ...
  <h2 id="titleTable">Perguntas Verificadas</h2>
        ...
        while ($dados = $consult->fetch_assoc()) { ?>
          <tbody>
            ...
            <th><?php $id_material = $dados['id_material'];
              ...
              <td> <a href="../controller/checaQuestao.php?id_questao=<?php echo $dados['id_questao'] ?>" class="btn btn-outline" style="background-color: #E49695; color:#ffffff;">Checar</td>
              <td> <a href="../views/editarQuestao.php?id_questao=<?php echo $dados['id_questao'] ?>" class="btn btn" style="background-color: #E49695;color:#ffffff;">Editar</td>
              <td> <a href="../controller/apagaQuestao.php?id_questao=<?php echo $dados['id_questao'] ?>" class="btn btn" style="background-color: #E49695;color:#ffffff;">Apagar</td>
            </tr>
          </tbody>
        <?php } ?>
      </table>
