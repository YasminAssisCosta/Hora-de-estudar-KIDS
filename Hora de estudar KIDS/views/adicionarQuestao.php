...
<body>
    ...
    <div class="row"> 
        <?php
    if (isset($_SESSION['erro_AQ'])) {
      echo "<div class='alert alert-danger' style='height: 40px, width:70%;'>
        $_SESSION[erro_AQ]
      </div> ";
      unset($_SESSION['erro_AQ']);
    }
    if (isset($_SESSION['sucesso_AQ'])) {
      echo "<div class='alert alert-success' style='height: 50px;'>
          $_SESSION[sucesso_AQ]
      </div> ";
      unset($_SESSION['sucesso_AQ']);
    }
    ?>
    </div>
        <form enctype="multipart/form-data" action="../controller/adicionarQuestao.php" method="POST" style="margin-left: 8%; max-width: 80%;">
        <input type="hidden" name="id_admin" value="<?php echo "$id"  ?>">
        <h2 style="margin-top: 20px; margin-left:35%">Adicione uma Questão:</h2><br>
...
<br>
        ...
        <select class="form-select" aria-label="Default select example" name="id_material">
            ...
            <?php while ($material = $consulta->fetch_assoc()) { ?>
            <option value="<?php echo $material['id_material'] ?>"><?php echo $material['titulo'] ?></option>
            <?php   } ?>
        </select></div><br>
